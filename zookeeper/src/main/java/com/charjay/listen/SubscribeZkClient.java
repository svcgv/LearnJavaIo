...
public class SubscribeZkClient {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            ZkClient clientManage = new ZkClient(Config.CONNECTSTRING, 5000, 5000, new BytesPushThroughSerializer());
            manageServer = new ManageServer(SERVERS_PATH, COMMAND_PATH, CONFIG_PATH, clientManage, initConfig);
            manageServer.start();

            for (int i = 0; i < CLIENT_QTY; ++i) {
                ZkClient client = new ZkClient(Config.CONNECTSTRING, 5000, 5000, new BytesPushThroughSerializer());
                clients.add(client);
                ServerData serverData = new ServerData();
                serverData.setId(i);
                serverData.setName("WorkServer#" + i);
                serverData.setAddress("192.168.98." + i);//此处替换为每台服务器自己的ip
